...
const RecherchePage = () => {
  ...
  const activeCount = [selectedCategory, minPrice, maxPrice, searchQuery, isSorted && ordering].filter(Boolean).length;
  ...
  return (
    <>
      <SEOHead
        title={searchQuery ? `Recherche — ${searchQuery}` : 'Toutes nos pièces'}
        description="Parcourez l'ensemble des créations Golden Pousso — Haute couture africaine à Dakar, Sénégal."
        url={searchQuery ? `/recherche?search=${encodeURIComponent(searchQuery)}` : '/recherche'}
      />

      <div style={{ background: '#FAF6EE', minHeight: '100vh', color: '#1A1208' }}>

        {/* ── Hero ── */}
        <section className="recherche-section" style={{ padding: '10rem 4rem 0', maxWidth: '130rem', margin: '0 auto' }}>
          <div className="recherche-title" style={{ marginBottom: '5rem', paddingLeft: 'calc(28rem + 5rem)' }}>
            <p style={{
              fontSize: '1.1rem', fontFamily: 'Inter, sans-serif',
              textTransform: 'uppercase', letterSpacing: '0.3em',
              color: '#B8960A', marginBottom: '1.6rem',
            }}>
              {searchQuery ? 'Recherche' : 'Boutique'}
            </p>
            <h1 style={{
              ...
              {searchQuery ? <>Résultats pour<br /><span style={{ color: '#B8960A' }}>« {searchQuery} »</span></> : 'Toutes Nos Pièces'}
            </h1>
          </div>
        </section>

        {/* ── Layout principal : produits + filtre sticky ── */}
        <div className="recherche-layout" style={{
          ...
          <aside style={{ position: 'sticky', top: '9rem', alignSelf: 'start' }}>
            ...
            {/* Prix */}
            <h3 style={{ fontFamily: 'Syne, sans-serif', fontSize: '1.6rem', color: '#B8960A', margin: '2.8rem 0 1.6rem', letterSpacing: '0.04em', fontWeight: 400 }}>
              Prix (FCFA)
            </h3>
            <PriceFilter
              minPrice={minPrice}
          ...
          <div>
            ...
            {/* Grille produits */}
            {loading ? (
              <div className="recherche-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2.4rem 2rem' }}>
      ...
      <style>{`
        ...
        @media (max-width: 1100px) {
          .recherche-layout { grid-template-columns: 1fr 24rem !important; gap: 3rem !important; }
          .recherche-grid   { grid-template-columns: repeat(2, 1fr) !important; }
        }
        @media (max-width: 860px) {
          .recherche-layout { grid-template-columns: 1fr !important; padding: 3rem 3rem 8rem !important; }
          .recherche-layout aside { position: static !important; max-height: none !important; }
          .recherche-title { padding-left: 0 !important; }
        }
        @media (max-width: 640px) {
          .recherche-section { padding-left: 2rem !important; padding-right: 2rem !important; }
        }
        @media (max-width: 560px) {
          .recherche-grid { grid-template-columns: 1fr !important; }
        }
